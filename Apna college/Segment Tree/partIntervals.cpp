//Same as findIntervals.cpp with slight modification here we have to find the number of 
//elements that are present only once

//Both this and findIntervals.cpp are not working for some test cases need to be checked
...
int main()
{
    ...
    for(int i=0;i<n/2;i++)
    {
        ans[t[i].idx-1]=(t[i].r-t[i].l-1)-2*find(1,0,n-1,t[i].l,t[i].r);    
