...
//Memoization
int Knapsack(int idx,int capacity,vector<int>& weight,vector<int>& value,vector<vector<int>>& dp){
    if(capacity==0)
        return 0;
    if(idx==0){
    ...
    int drop=Knapsack(idx-1,capacity,weight,value,dp),take=0;
    if(weight[idx]<=capacity)
        take=Knapsack(idx-1,capacity-weight[idx],weight,value,dp)+value[idx];
    return dp[idx][capacity]=max(drop,take);
}

//Tabulation
int Knapsack(int capacity,vector<int>& weight,vector<int>& value,vector<vector<int>>& dp){
    int n=weight.size();
...
int main(){
    ...
    cout<<"The maximum possible value to steal is: "<<Knapsack(capacity,weight,value);
}
